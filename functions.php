<?php
/**
* greenfinch functions and definitions
...
function greenfinch_scripts() {

    // load Bootstrap CSS v5.1
    wp_enqueue_style( 'greenfinch-bootstrap-css', THEME_DIR_URI . '/includes/css/bootstrap.min.css');

    // load website CSS, versioned
    wp_enqueue_style( 'greenfinch-css', THEME_DIR_URI . '/assets/css/style.css', [], filemtime( get_stylesheet_directory() . '/assets/css/style.css') );

	// load Bootstrap JS v5.1
	wp_enqueue_script( 'greenfinch-bootstrap-js', THEME_DIR_URI . '/includes/js/bootstrap.min.js', array('jquery') );
}
add_action( 'wp_enqueue_scripts', 'greenfinch_scripts' );
...
/* Register menus */
if ( ! function_exists( 'greenfinch_setup' ) ) :
	function greenfinch_setup() {

    register_nav_menus( array(
        'primary' => __( 'Primary Menu', 'greenfinch' ),
        'footer'  => __( 'Footer Menu', 'greenfinch' ),
    ) );
...
add_action( 'after_setup_theme', 'greenfinch_setup');
